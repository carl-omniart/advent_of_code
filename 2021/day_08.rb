...
file_path = Dir.pwd + "/day_08_input.txt"
input     = File.readlines file_path

displays = input.map { |entry| Display.new entry }
count    = displays.map { |display| display.unique_output_count }.sum

puts "1. In the output values, there are #{count} instances of a digits that use a unique number of segments." 

sum = displays.map { |display| display.output_value.to_i }.sum

puts "2. Adding up all the output values equals #{sum}."